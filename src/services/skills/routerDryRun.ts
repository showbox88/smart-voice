...
R2. SKILL SELECTION — pick the RIGHT skill, never substitute.
    ...
    Calendar (calendar_query + calendar_create + calendar_cancel — only when in catalog):
    ...
    - 把明天的安排取消 / 取消今天的会议 / cancel tomorrow's meetings / delete lunch
                                               → calendar_cancel (range + optional match + all)
    - CRITICAL for calendar_cancel: the when field MUST be "now" — the cancel
      ACTION runs immediately. 日期信息("今天"/"明天"/"本周") 属于 range 槽位，不是 when。
      把「明天的会议取消」这类语句拆成 slots.range="tomorrow", when="now"。
      填 when="明天" 会触发 unparseable 错误。
    - calendar_cancel 的槽位:
        range: today/tomorrow/week/next（默认 today；只报标题不报时间时用 week）
        match: 标题模糊匹配（可选）— 用户点名的时候填，如"跟张三的会" → match:"张三"
        all: true 只在用户明说"全部/都/所有/all"时填；否则默认 false，由 handler 处理歧义
    - DIFFERENCE vs reminder:
      ...
      Use calendar_cancel when user says "取消"/"删掉"/"cancel"/"delete" about an event.
...
R13. WINDOWS_CONTROL 是**兜底** skill —— 只在 R1–R12 里没有一条命中**具体**
    skill 时才选。不要抢其他 skill 的活：
      ...
      - 日历             → calendar_query / calendar_create / calendar_cancel
      - 发消息           → messaging
      - 提醒             → reminder
      - 天气/知识问答    → info / chat
